...
namespace G2T.NCD.Table {
    public class ItemTable : TableConfig<ItemTable> {
#if UNITY_EDITOR
        ...
        private void LoadData() {
            ...
            for(int i = 0; i < colsRow.LastCellNum; i++) {
                ...
                if(cell.StringCellValue == idCol) idIndex = i;
                if(cell.StringCellValue == nameCol) nameIndex = i;
                if(cell.StringCellValue == descriptionCol) descriptionIndex = i;
            }
            for(int i = sheet.FirstRowNum + 1; i < sheet.LastRowNum; i++) {
                ...
                if(header.StringCellValue == "END")
                    break;
                var idCell = GetCell(row, idIndex);
                var nameCell = GetCell(row, nameIndex);
                var descriptionCell = GetCell(row, descriptionIndex);

                var item = new ItemInfo(Convert.ToInt32(idCell.NumericCellValue), nameCell.StringCellValue, descriptionCell.StringCellValue);
